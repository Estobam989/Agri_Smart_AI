...
# For Streamlit deployment, API keys should be in st.secrets
try:
    ANTHROPIC_API_KEY = st.secrets["ANTHROPIC_API_KEY"]
except KeyError:
    st.error("ANTHROPIC_API_KEY not found in Streamlit secrets. Please add it to your Streamlit Cloud secrets.")
    st.stop()

# Model configuration directly within app.py for deployment consistency
MODEL_NAME = "claude-3-5-sonnet-20240620" # Explicitly set for deployment
TEMPERATURE = 0.7 # Default temperature
MAX_TOKENS = 8192 # Default max tokens

if not ANTHROPIC_API_KEY or ANTHROPIC_API_KEY == "sk-YOUR_ANTHROPIC_API_KEY":
    st.error("ANTHROPIC_API_KEY not found in Streamlit secrets or is still a placeholder. Please set it correctly.")
    st.stop()

# Set ANTHROPIC_API_KEY environment variable (some libraries might use it directly)
